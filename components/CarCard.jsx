...
import { useAuth } from "@/context/AuthContext"; // assume this gives { user }
      ...
      <div className="p-6">
        ...
        <div className="flex space-x-6">
          <Link
            href={
              user
                ? `/bookings/create?carId=${car._id.toString()}`
                : "/users/register"
            }
            className="flex-1 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-center py-3 rounded-xl hover:bg-blue-50 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
          >
            Book Now
          </Link>

          {/* <Link href={`/bookings/create?carId=${car.id}`}>Book Now</Link> */}
        </div>
      </div>
    </div>
  );
